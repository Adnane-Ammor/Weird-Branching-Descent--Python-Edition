...
def story_node1_choice():
    global ALIVEP_choice
    
    while True:
        dead_parent = input("\nFive years later, God himself comes to you in a dream and gives you the choice to either kill your mom or dad [mom | dad]: ").lower()
        if dead_parent in ["mom", "dad"]:
            break
        print("Invalid choice. Please type 'mom' or 'dad'.")
...
B. Rest on the ground for a bit? [A | B]: ''').upper()
        if choice_B in ["A", "B"]:
            break
...
def story_node2_choice():
    global bully_standoff
    
    input("\nA. Do you keep staring at him?")
    input("B. do you punch him?")
    
    while True:
        bully_standoff = input("or C. do you run off and tell a trusted adult? [A | B | C]: ").upper()
        if bully_standoff in ["A", "B", "C"]:
            break
...
def ending():
    ...
    while True:
        adv_choice = input("Advance forward? [y | n]: ").lower()
        if adv_choice in ["y", "n"]:
            break
        print("Invalid input. Please type 'y' or 'n'.")
        
    if adv_choice == "y":
        input("(ya know it doesn't matter which one you choose you were gonna go up anyway...") 
        input("(what?! did you think this was Life is Strange?)")
    elif adv_choice == "n":
        input("(didn't I already tell you that we ran out of funding? Do you think this is Life is Strange?)")
        input("(we don't have time to explore!")
...
try:
    ...
    while not name.strip():
        name = input("Name cannot be empty. Name your character: ")

    while True:
        pronoun_choice = input("What pronouns? [he | she | they]: ").lower()
        if pronoun_choice in PRONOUNS:
            break
        print("Invalid pronouns. Please choose from: he, she, or they.")
    ...
    print("\nKeyboardInterrupt: SEE YA LATER :P")